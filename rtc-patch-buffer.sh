...
echo "🔧 Patch RTC : tampon ICE + flush + vérif socket.emit"
echo "------------------------------------------------------"
...
echo "📦 Sauvegarde → $TARGET.old"
cp "$TARGET" "$TARGET.old"

echo "📦 Application du patch"
patch "$TARGET" < "$PATCH"

# 3. Vérification socket.emit("partner")
echo "🔍 Vérification dans $SERVER..."
if grep -q 'socket.emit("partner"' "$SERVER"; then
  echo "✅ socket.emit(\"partner\") déjà présent"
else
  echo "⚠️ socket.emit(\"partner\") absent — à ajouter après le pairing"
fi
