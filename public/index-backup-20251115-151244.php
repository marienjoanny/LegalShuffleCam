...
<body>
  ...
  <script>
    ...
    async function detectCameras() {
      log("Détection des caméras en cours...");
      try {
        const permissionStatus = await navigator.permissions.query({ name: 'camera' });
        log(\`Permissions: \${permissionStatus.state}\`);
        if (permissionStatus.state === 'denied') {
          log("Accès caméra refusé. Autorisez dans les paramètres.", true);
          return;
        }
        ...
        if (cameras.length > 0) {
          ...
          log("Aucune caméra videoinput détectée", true);
        }
