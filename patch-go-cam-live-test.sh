#!/bin/bash
echo "🔧 Correction du script go-cam-live-test.sh avec argument TOKEN"

TARGET="go-cam-live-test.sh"
BACKUP="${TARGET}.pre-arg-patch.bak"

# Sauvegarde
cp "$TARGET" "$BACKUP"
echo "🗂️  Backup enregistré : $BACKUP"

# Injection du contenu corrigé
cat <<'SH' > "$TARGET"
#!/bin/bash
...
chmod +x "$TARGET"
echo "✅ Script corrigé : tu peux maintenant faire ./go-cam-live-test.sh <TOKEN>"
